...
public class JdbcTestResultBeanDAO extends JdbcAbstractDAO<TestResultBean> implements TestResultBeanDAO {
    ...
    private static final String SQL__SELECT_BY_USER_ID = "SELECT TESTS.NAME, TESTS.SUBJECT, RESULTS.RESULT FROM TESTS " +
            "JOIN RESULTS ON TESTS.ID = RESULTS.TEST_ID JOIN USERS ON RESULTS.USER_ID = USERS.ID WHERE USERS.ID = ?";
